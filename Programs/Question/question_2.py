        ...
        for row in matches_data:
            if row["result"] == "normal":
                if row["winner"] in winner_team_dict:
                    if int(row["season"]) in winner_team_dict[row["winner"]]:
                        winner_team_dict[row["winner"]][int(row["season"])]+=1
                    else:
                        winner_team_dict[row["winner"]][int(row["season"])]=1
                else:
...
def plot(winner_team_dict):
    ...
    plt.savefig("../Figures/question_2_fig.png",bbox_inches='tight')  #Saving the image in different location
    plt.show()


if __name__=="__main__":
    winner_team_dict=execute()
    plot(winner_team_dict)