                ...
                if row["bowler"] in economy_data_dict:
                    ...
                    if economy_data_dict[row["bowler"]][2] != 0:
                        economy_data_dict[row["bowler"]][3] = round(
                            economy_data_dict[
                                row["bowler"]
                            ][0] / economy_data_dict[row["bowler"]][2],
                            1)
                    else:
...
def transform(economy_data_dict):
    ...
    """
    sorted_economy_data_dict = dict(sorted(
        economy_data_dict.items(),
        key=lambda item: item[1][3])
    )
    bowlers_list = list(sorted_economy_data_dict.keys())
    economy_list = []
    i = 0
